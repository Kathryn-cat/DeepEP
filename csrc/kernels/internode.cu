...
get_num_bytes_per_rdma_token(int hidden_int4, int num_scales, int num_topk_idx,
...
get_rdma_clean_meta(int hidden_int4, int num_scales, int num_topk_idx,
                    int num_topk_weights, int num_rdma_ranks,
                    int num_rdma_recv_buffer_tokens, int num_sms) {
  // Return `int32_t` offset and count to clean
  return {(get_num_bytes_per_rdma_token(hidden_int4, num_scales, num_topk_idx,
                                        num_topk_weights) *
...
__host__ __device__ __forceinline__ std::pair<int, int>
get_nvl_clean_meta(int hidden_int4, int num_scales, int num_topk_idx,
                   int num_topk_weights, int num_rdma_ranks, int num_nvl_ranks,
                   int num_nvl_recv_buffer_tokens, int num_sms) {
...
nvshmem_sync_with_same_gpu_idx(const nvshmem_team_t &rdma_team) {
  // internode sync
...
__global__ void notify_dispatch(
    const int *num_tokens_per_rank, int *moe_recv_counter_mapped, int num_ranks,
...
__global__ void
__launch_bounds__(((kNumDispatchRDMASenderWarps + 1 + NUM_MAX_NVL_PEERS) * 32),
                  1)
    dispatch(int4 *recv_x, float *recv_x_scales, int64_t *recv_topk_idx,
             ...
             const int *recv_gbl_rank_prefix_sum, const bool *is_token_in_rank,
  ...
  auto nvl_channel_src_meta =
      AsymBuffer<SourceMeta>(ws_rr_buffer_ptr, num_max_nvl_chunked_recv_tokens,
  ...
  if (warp_role == WarpRole::kRDMASender) {
    ...
    for (token_idx = token_start_idx + warp_id; token_idx < token_end_idx;
      ...
      if (last_rdma_tail_idx >= 0)
        st_release_cta(
            const_cast<const int *>(rdma_send_channel_tail + lane_id),
...
#pragma unroll
      for (int i = lane_id; i < num_topk * num_topk_ranks; i += 32) {
        ...
        st_na_global(reinterpret_cast<int *>(dst_send_buffers[rank_idx]) +
                         copy_idx,
                     idx_value);
    ...
    while (__any_sync(0xffffffff, num_tokens_to_send > 0)) {
      ...
      if (clock64() - start_time > NUM_TIMEOUT_CYCLES and
        ...
        printf("DeepEP RDMA sender coordinator timeout, channel: %d, IB: %d, "
               "nvl %d, dst IB: %d, tail %d, num_tokens_to_send %d\n",
    ...
    if (lane_id < kNumRDMARanks) {
      while (true) {
        ...
        if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
          printf("DeepEP dispatch forwarder timeout (RDMA meta), channel: %d, "
    ...
    while (__any_sync(0xffffffff, num_tokens_to_recv_from_rdma > 0)) {
      ...
      while (lane_id == 0) {
        ...
        // Timeout check
        if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
          printf("DeepEP dispatch forwarder timeout (NVL check), channel: %d, "
      ...
      for (int i = src_rdma_head, num_tokens_sent = 0; i < src_rdma_tail; ++i) {
        ...
        if (lane_id < num_topk) {
          ...
          auto weight_value =
              ld_nc_global(static_cast<float *>(shifted) + lane_id);
          ...
          idx_value = (idx_value >= dst_rank_expert_begin and
                       idx_value < dst_rank_expert_end)
                          ...
                          : -1;
          st_na_global(nvl_channel_topk_idx.buffer() + dst_slot_idx * num_topk +
                           lane_id,
                       idx_value);
...
#pragma unroll
    ...
    while (lane_id < kNumRDMARanks) {
      ...
      // Timeout check
      if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
        printf("DeepEP dispatch NVL receiver timeout, channel: %d, RDMA: %d, "
               "nvl: %d, src RDMA: %d, src nvl: %d, start: %d, end: %d\n",
    ...
    while (num_tokens_to_recv > 0) {
      ...
      for (int chunk_idx = 0; chunk_idx < num_recv_tokens;
        ...
        // Copy data
        UNROLLED_WARP_COPY(
            5, lane_id, hidden_int4, recv_x + recv_token_idx * hidden_int4,
        ...
        // Copy scales
        UNROLLED_WARP_COPY(
            1, lane_id, num_scales, recv_x_scales + recv_token_idx * num_scales,
...
void dispatch(void *recv_x, float *recv_x_scales, int64_t *recv_topk_idx,
              float *recv_topk_weights, void *recv_src_meta, const void *x,
              const float *x_scales, const int64_t *topk_idx,
              const float *topk_weights, int *send_rdma_head,
...
template <bool kLowLatencyMode>
__global__ void
cached_notify(const int rdma_clean_offset, const int rdma_num_int_clean,
              const int nvl_clean_offset, const int nvl_num_int_clean,
              int *combined_rdma_head, int num_combined_tokens,
              int num_channels, const int *rdma_channel_prefix_matrix,
              const int *rdma_rank_prefix_sum, int *combined_nvl_head,
...
template <int kNumRanks, typename dtype_t, int kMaxNumRanks, typename ReceiveFn,
...
__global__ void __launch_bounds__((NUM_MAX_NVL_PEERS + 1 + kNumForwarders) * 32,
    ...
    combine(int4 *combined_x, float *combined_topk_weights,
            const bool *is_combined_token_in_rank, const int4 *x,
            const float *topk_weights, const int *combined_rdma_head,
            const int *combined_nvl_head, const SourceMeta *src_meta,
            const int *rdma_channel_prefix_matrix,
            const int *rdma_rank_prefix_sum,
            const int *gbl_channel_prefix_matrix, int num_tokens,
  ...
  if (warp_role == WarpRole::kNVLSender) {
    ...
    while (true) {
      ...
      while (true) {
        ...
        if (clock64() - start_time > NUM_TIMEOUT_CYCLES and
            lane_id < kNumRDMARanks) {
          printf("DeepEP combine NVL sender timeout, channel: %d, RDMA: %d, "
                 "nvl: %d, dst NVL: %d, RDMA lane: %d, head: %d, tail: %d, "
                 "start: %d, end: %d\n",
                 channel_id, rdma_rank, nvl_rank, dst_nvl_rank, lane_id,
                 ld_volatile_global(nvl_channel_head.buffer() + lane_id),
      ...
      for (int current_rdma_idx = 0; current_rdma_idx < kNumRDMARanks;
           ++current_rdma_idx) {
        if (__shfl_sync(0xffffffff,
                        (token_start_idx >= token_end_idx) or
                            (not is_lane_ready),
                        current_rdma_idx))
          continue;
        ...
        for (int chunk_idx = 0; chunk_idx < num_tokens_in_chunk;
          ...
          UNROLLED_WARP_COPY(5, lane_id, hidden_int4, shifted_x_buffers,
    ...
    if (warp_role == WarpRole::kNVLAndRDMAForwarder) {
      ...
      for (int token_start_idx = 0; token_start_idx < num_tokens_to_combine;
        ...
        while (sub_warp_id == 0 and lane_id == 0) {
          ...
          // Timeout check
          if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
            printf("DeepEP combine forwarder (RDMA check) timeout, channel: "
                   "%d, RDMA: %d, nvl: %d, dst RDMA: %d, head: %ld, tail: %d, "
                   "chunked: %d\n",
      ...
      for (int64_t token_idx = token_start_idx + warp_id;
        ...
        // Combine current token
        auto recv_fn = [&](int src_rdma_rank, int slot_idx,
                           int hidden_int4_idx) -> int4 {
...
void combine(cudaDataType_t type, void *combined_x,
             float *combined_topk_weights,
             const bool *is_combined_token_in_rank, const void *x,
             const float *topk_weights, const int *combined_rdma_head,
...
#define COMBINE_LAUNCH_CASE(num_rdma_ranks)                                    \
  {                                                                            \
    ...
    LAUNCH_KERNEL(                                                             \
        ...
        reinterpret_cast<const int4 *>(x), topk_weights, combined_rdma_head,   \
        combined_nvl_head, reinterpret_cast<const SourceMeta *>(src_meta),     \
