...
// 2. dispatch (skip notify_dispatch)

template <bool kUseFP8, bool kUseUE8M0, int kNumWarpGroups,
...
__launch_bounds__(kNumWarpGroups *kNumWarpsPerGroup * 32, 1) void dispatch(
    ...
    int *cumulative_local_expert_recv_stats, void *rdma_recv_x,
    int *rdma_recv_count, void *rdma_x, const void *x, const int64_t *topk_idx,
    ...
    int rank, int num_ranks, bool round_scale, int *usage_flag, int phases) {
      ...
      if (dst_expert_idx >= 0) {
        ...
        if (dst_p2p_ptr == 0) {
          nvshmemi_ibgda_put_nbi_warp(dst_ptr, src_ptr, num_bytes_per_msg,
                                      dst_rank, dst_expert_local_idx, lane_id,
                                      slot_idx);
        } else {
...
#pragma unroll
  ...
  if (responsible_expert_idx < num_experts and sub_warp_id == 0 and
    ...
    if (dst_p2p_ptr == 0) {
      nvshmemi_ibgda_amo_nonfetch_add(reinterpret_cast<int *>(dst_ptr),
...
LOW_LATENCY_DISPATCH_RECV:
  ...
  if (responsible_expert_idx < num_experts) {
    ...
    const auto num_aligned_scales =
        align<int>(num_scales, sizeof(float) / sizeof(scale_t));
    const auto recv_x_scales =
        reinterpret_cast<scale_t *>(packed_recv_x_scales) +
        local_expert_idx * num_ranks * num_max_dispatch_tokens_per_rank *
            num_aligned_scales;
...
void dispatch(void *packed_recv_x, void *packed_recv_x_scales,
              ...
              const void *x, const int64_t *topk_idx, int *next_clean,
              ...
              int *usage_flag, cudaStream_t stream, int phases) {
...
#define DISPATCH_LAUNCH_CASE(hidden)                                           \
  {                                                                            \
    ...
    LAUNCH_KERNEL(&cfg, dispatch_func, packed_recv_x, packed_recv_x_scales,    \
                  ...
                  rank, num_ranks, round_scale, usage_flag, phases);           \
  }                                                                            \
  break
...
template <int kNumWarpGroups, int kNumWarpsPerGroup, int kHidden,
          int kNumMaxTopk>
__global__
__launch_bounds__(kNumWarpGroups *kNumWarpsPerGroup * 32, 1) void combine(
    void *combined_x, void *rdma_recv_x, int *rdma_recv_flag, void *rdma_send_x,
    const void *x, const int64_t *topk_idx, const float *topk_weights,
    const int *src_info, const int64_t *layout_range, int *next_clean,
    ...
    int num_experts, int rank, int num_ranks, int *usage_flag, int phases,
    bool zero_copy) {
  const auto sm_id = static_cast<int>(blockIdx.x);
  const auto num_sms = static_cast<int>(gridDim.x);
  const auto thread_id = static_cast<int>(threadIdx.x);
  ...
  if (responsible_expert_idx < num_experts) {
    ...
    for (int token_idx = offset + sub_warp_id;
      ...
      if (dst_p2p_ptr == 0) {
        ...
        if (not zero_copy)
          UNROLLED_WARP_COPY(7, lane_id, hidden_bf16_int4, buf_int4_ptr, x_int4,
                             ld_nc_global, st_na_global);
        ...
        const auto dst_int4_ptr = reinterpret_cast<int4 *>(dst_p2p_ptr);
        UNROLLED_WARP_COPY(7, lane_id, hidden_bf16_int4, dst_int4_ptr, x_int4,
                           ld_nc_global, st_na_global);
      }
    }
