...
template <int kNumRanks> // spec: number of ranks
__global__ void notify_dispatch(
    const int *num_tokens_per_rank, // input: (num_ranks,), in launching GPU's
                                    // memory, how many tokens this GPU will
                                    // send to every other rank
    int *moe_recv_counter_mapped, // output: (1,), in host-mapped memory, total
                                  // number of tokens this GPU will receive
    const int
        *num_tokens_per_expert, // input: (num_experts,), in launching GPU's
                                // memory, how many tokens this GPU will send to
                                // each specific expert in the system
    int *moe_recv_expert_counter_mapped, // output: (num_local_experts,), in
                                         // host-mapped memory, the total number
                                         // of tokens each local expert on this
                                         // GPU will receive
    int num_experts,  // spec: total number of experts in the system
    int num_tokens,   // spec: total number of tokens in this batch on this GPU
    int num_channels, // spec: The number of parallel communication channels to
                      // use, typically num_sms / 2. This determines the degree
                      // of parallelism in the dispatch kernel.
    const bool
        *is_token_in_rank, // input: (num_tokens, num_ranks), indicate if
                           // each token needs to be sent to a given rank, used
                           // by the channel workload calculation
    int *channel_prefix_matrix,   // output: (num_ranks, num_channels), final
                                  // prefix sum for each communication
                                  // channel for each destination rank.
    int *rank_prefix_matrix_copy, // output: (num_ranks, num_ranks),
                                  // final, globally-agreed-upon prefix sum
                                  // table for token placement
    int num_memset_int,   // spec: The number of integers to zero-out in the
                          // shared buffer to prepare it for the next kernel.
    int expert_alignment, // spec: 	A value to align the number of tokens
                          // per expert to, often for performance reasons in
                          // subsequent GEMM operations.
    void **buffer_ptrs, // tool: (num_ranks,), i-th is shared comm buffer of GPU
                        // i, the key to inter-GPU memory access
    int **barrier_signal_ptrs, // tool: (num_ranks,), for barrier_block sync
                               // primitive
    int rank                   // input: integer id of this GPU
) {
  ...
  if (sm_id == 0) {
    ...
    int *per_rank_buffer;   // temp: (num_ranks, num_ranks), within sm_id == 0,
                            // register-level, used by a GPU to write to another
                            // GPU's buffer
    int *per_expert_buffer; // temp: (num_ranks, num_experts_per_rank), within
                            // sm_id == 0, register-level, used by a GPU to
                            // write to another GPU's buffer
    ...
    int num_experts_per_rank =
        num_experts / kNumRanks; // number of experts hosted on a single GPU.
    if (thread_id < kNumRanks) {
#pragma unroll
...
#pragma unroll
    ...
    auto local_per_rank_buffer =
        static_cast<int *>(buffer_ptrs[rank]); // temp: a pointer to curr GPU's
                                               // own shared comm buffer
    if (thread_id < kNumRanks) {
#pragma unroll
...
#pragma unroll
    ...
    // other SMs
    int dst_rank = sm_id - 1; // dest rank the current SM is responsible for
    for (int channel_id = warp_id; channel_id < num_channels;
      ...
      // Iterate over tokens
      int count = 0; // temp: per-warp accumulator
...
void notify_dispatch(const int *num_tokens_per_rank,
                     ...
                     int num_tokens, const bool *is_token_in_rank,
...
#define NOTIFY_DISPATCH_LAUNCH_CASE(ranks)                                     \
  ...
  SETUP_LAUNCH_CONFIG(1 + num_ranks, kNumThreads,
                      stream); // launch: (#ranks + 1) SMs
  SWITCH_RANKS(NOTIFY_DISPATCH_LAUNCH_CASE);
#undef NOTIFY_DISPATCH_LAUNCH_CASE
}

// 1.5  cached_notify_dispatch
...
__global__ void __launch_bounds__(kNumThreads, 1)
    dispatch(int4 *recv_x, float *recv_x_scales, int *recv_src_idx,
             ...
             const int4 *x, // input: tensor of all tokens
             const float *x_scales, const int64_t *topk_idx,
             const float *topk_weights, const bool *is_token_in_rank,
             const int *channel_prefix_matrix, int num_tokens,
  ...
  // sender is on even-numbered SMs
...
void dispatch(void *recv_x, float *recv_x_scales, int *recv_src_idx,
              int64_t *recv_topk_idx, float *recv_topk_weights,
              int *recv_channel_offset, int *send_head, const void *x,
              const float *x_scales, const int64_t *topk_idx,
              const float *topk_weights, const bool *is_token_in_rank,
              const int *channel_prefix_matrix, int num_tokens,
              int num_worst_tokens, int hidden_int4, int num_topk,
...
#define DISPATCH_LAUNCH_CASE(ranks)                                            \
  {                                                                            \
    ...
    LAUNCH_KERNEL(                                                             \
        ...
        send_head, reinterpret_cast<const int4 *>(x), x_scales, topk_idx,      \
...
__global__ void __launch_bounds__(kNumThreads, 1)
    combine(dtype_t *recv_x, float *recv_topk_weights, const dtype_t *x,
            const float *topk_weights, const int *src_idx,
            const int *rank_prefix_matrix, const int *channel_prefix_matrix,
  ...
  if (is_sender) {
    ...
    for (int64_t token_idx = token_start_idx; token_idx < token_end_idx;) {
      ...
      while (lane_id == 0) {
        ...
        // Rare cases to loop again
        if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
          printf("DeepEP timeout for combine senders, rank %d, "
                 "responsible_channel = %d\n",
                 rank, responsible_channel);
          trap();
        }
...
#pragma unroll
      for (int i = 0; i < kNumRanks; ++i) {
        ...
        auto ptr = reinterpret_cast<void *>(
            static_cast<int8_t *>(buffer_ptrs[rank]) +
      ...
      for (int64_t token_idx = token_start_idx + recv_warp_id - 1;
        ...
        auto start_time = clock64();
        while (channel_tail_idx[lane_id] <= expected_head and
               expected_head >= 0) {
          // Timeout check
          if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
            printf("DeepEP timeout for combine receivers, rank %d, "
...
void combine(cudaDataType_t type, void *recv_x, float *recv_topk_weights,
             const void *x, const float *topk_weights, const int *src_idx,
             const int *rank_prefix_matrix, const int *channel_prefix_matrix,
             int *send_head, int num_tokens, int num_recv_tokens, int hidden,
...
#define COMBINE_LAUNCH_CASE(dtype, ranks)                                      \
  {                                                                            \
    ...
    LAUNCH_KERNEL(&cfg, kernel, reinterpret_cast<dtype *>(recv_x),             \
                  ...
                  topk_weights, src_idx, rank_prefix_matrix,                   \
                  channel_prefix_matrix, send_head, num_tokens,                \
                  num_recv_tokens, hidden, num_topk, buffer_ptrs, rank,        \
                  num_max_send_tokens, num_recv_buffer_tokens);                \
  }                                                                            \
  break
